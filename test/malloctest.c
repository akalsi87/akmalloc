/* from: https://cgi.cse.unsw.edu.au/~cs3231/04s2/work/src/testbin/malloctest/malloctest.c
 * malloctest.c
 *
...
test2(void)
{
    ...
    for (size = HUGESIZE; (x = ak_malloc(size))==NULL; size = size/2) {
...
test3(void)
{
    // disabled because it takes too long and may crash
    return;
    ...
    while ((tmp = ak_malloc(sizeof(struct test3))) != NULL) {
    ...
    x = ak_malloc(SMALLSIZE);
    if (x != NULL) {
        printf("FAILED: ak_malloc(%u) succeeded\n", SMALLSIZE);
        return;
    }

    x = ak_malloc(MEDIUMSIZE);
    if (x != NULL) {
        printf("FAILED: ak_malloc(%u) succeeded\n", MEDIUMSIZE);
        return;
    }
...
test4(void)
{
    ...
    x = ak_malloc(SMALLSIZE);
    if (x==NULL) {
        printf("FAILED: ak_malloc(%u) failed\n", SMALLSIZE);
        return;
    }

    y = ak_malloc(MEDIUMSIZE);
    if (y==NULL) {
        printf("FAILED: ak_malloc(%u) failed\n", MEDIUMSIZE);
        return;
    }
